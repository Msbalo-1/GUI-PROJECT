...
def equal_to():
	...
	if function == 'add':
		...
		field.insert(0, first_value / second_value)
#LABELS
field=Entry(root,width=40, bg ='blue', fg='white')

#BUTTONS
num0 = Button(root, padx=width,pady=height, text='0',command=lambda: value(0))
...
equal = Button(root, padx=width,pady=height, text='=',command=equal_to)
clear = Button(root, padx=width,pady=height, text='clr',command=clear)
xterm= Button(root, padx=71,pady=30, text='exit',command=root.quit)
...
clear.grid(row=5 , column=1, columnspan=3 )
xterm.grid(row=4 , column=3, rowspan=2)
